...
// Your SecureGate AI Gateway (Default to local backend, override via environment variable)
const GATEWAY = process.env.GATEWAY_URL || "http://localhost:8080";

// API key for the System tenant
const API_KEY = "sg_live_default_system_secret_key_123456";

// Helper function to render the blocked page template
function renderBlockedPage(visitorIp, reason) {
    return `
        <!DOCTYPE html>
        <html>
        <head>
        ...
        <body>
            <div class="card">
                ...
                <p>Reason: ${reason || "IP Blacklisted due to security violations"}</p>
...
app.use(async (req, res, next) => {
    ...
    try {
        const response = await axios.get(
            ...
            {
                headers: {
                    'X-Visitor-Path': req.path,
                    'X-Visitor-User-Agent': req.headers['user-agent']
                }
            }
        );
        ...
        if (response.data.allowed) {
            ...
            return res.status(403).send(renderBlockedPage(visitorIp, response.data.reason));
        }

    } catch (err) {
        if (err.response && err.response.status === 403) {
            console.log(`[SecureGate AI] IP: ${visitorIp} -> BLOCKED (Gateway 403 Response)`);
            return res.status(403).send(renderBlockedPage(visitorIp, err.response.data?.reason));
        }
...
// GET /: Renders Customer Bank Login Page
app.get("/", (req, res) => {
    ...
    res.send(`
        ...
        <head>
            ...
            <style>
                ...
                .ip-footer { text-align: center; margin-top: 30px; font-size: 0.8rem; color: #475569; }
                .ip-val { color: #38bdf8; font-family: monospace; }
            </style>
        </head>
        <body>
            <div class="card">
...
// POST /login: Authenticate credentials (customer simulation triggers always fail)
app.post("/login", async (req, res) => {
    const visitorIp = req.visitorIp;

    try {
        const response = await axios.post(`${GATEWAY}/api/simulate/failed-login`, {
            ip: visitorIp,
            apiKey: API_KEY
        });

        if (response.data.currentStatus === "BLOCKED") {
            return res.json({
                status: "BLOCKED",
...
app.get("*", (req, res) => {
    ...
    res.send(`
        ...
        <html>
        <head>
            <title>Page Not Found - ABC Bank</title>
            <style>
                body { background-color: #080c14; color: #fff; font-family: system-ui, -apple-system, sans-serif; display: flex; align-items: center; justify-content: center; height: 100vh; margin: 0; }
                .card { background: rgba(15, 23, 42, 0.65); border: 1px solid rgba(255, 255, 255, 0.08); border-radius: 20px; padding: 40px; text-align: center; max-width: 450px; box-shadow: 0 10px 30px rgba(0,0,0,0.5); }
                h1 { color: #94a3b8; font-size: 2rem; margin-top: 0; }
                p { color: #64748b; line-height: 1.5; }
                .path { font-family: monospace; background: rgba(255,255,255,0.05); padding: 3px 6px; border-radius: 4px; color: #f43f5e; }
            </style>
        </head>
        <body>
            <div class="card">
                <h1>404 Page Not Found</h1>
                <p>The page <span class="path">${req.path}</span> does not exist on this secure server.</p>
                <p style="font-size: 0.85rem; color: #475569;">Visitor IP: ${visitorIp}</p>
            </div>
        </body>
        </html>
    `);
